...
def clean_url_picture_list(x):
    try:
        return re.findall("url\\(\"(.+?)\"\\)", str(x))[0].replace("size=small", "size=phare") if "url" in str(x) else str(x).replace("size=small", "size=phare")
    except Exception:
        logging.error(x)
        return np.nan
...
class CleanDrouot(TextCleaner):
    ...
    def clean_auctions(self, df_auctions):
        df_auctions = df_auctions.drop_duplicates([self.name.url_auction, self.name.auction_title, self.name.date])
    ...
    def clean_items_per_auction(self, df):
        ...
        df[self.name.localisation] = np.where(df[self.name.place].isin(["www.drouot.com", 'Drouot Live ONLY - - -', 'Vente Huis Clos -']), "Hotel Drouot - 75009 Paris",
                                     np.where(df[self.name.place].apply(lambda x: "www.setdart.com" in str(x)), "Arago, 346 - 08009 Barcelone, Espagne",  
                                     ...
                                     np.where(df[self.name.place]=="- , Royaume-Uni", "XXX - 001 London, Royaume uni",     
                                            df[self.name.place])))))
        df[self.name.localisation] = df[self.name.localisation].apply(lambda x: str(x).split(" - ")[-1].split(",")[0].strip().split(" ")[-1])
        ...
        df[self.name.id_auction] = df[self.name.item_file].apply(lambda x: str(x).replace(".csv", ""))

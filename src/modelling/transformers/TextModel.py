...
def formatting_prompts_func(PROMPT, examples):
    ...
    for i in range(len(examples['ID_ITEM'])):
        output_texts.append(prompt_llama3(PROMPT, 
                                            examples["input"][i], 
                                            examples["output"][i]))
    return output_texts
...
class TextModel(Step):
    ...
    def run_from_qlora_saved(self):
        ...
        self.merge_model(origin_model_name=self.origin_model_name,
    ...
    def check_model_in_huggingface(self, model_name):
        try:
            ...
            self._log.error(f"NEW MODEL ON HUGGINGFACE does not exist, check adldl/{model_name}")
    ...
    def run_llama_cpp_quantization(self):
        ...
        cmd = subprocess.Popen(["python", "C:/Users/alarr/Documents/repos/llama.cpp/convert.py", model_path_name, "--outfile", f"{model_path_name}/{self.model_new_name}.fp16.bin", "--outtype", "f16", "--vocab-type", "bpe"])
        cmd.communicate()

        cmd = subprocess.Popen(["C:/Users/alarr/Documents/repos/llama.cpp/build/bin/Release/quantize", f"{model_path_name}/{self.model_new_name}.fp16.bin", f"{model_path_name}/{self.model_new_name}-q4_k_m.gguf", "q4_K_M"])
        cmd.communicate()

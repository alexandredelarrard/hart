            ...
            email_news = (
                db.session.query(
                    cast(
                        func.date_trunc(
                            "day",
                            func.to_timestamp(
                                ActivityLog.activity_timestamp, "YYYY-MM-DD HH24:MI:SS"
                            ),
                        ),
                        Date,
                    ).label("date"),
                    func.sum(
                        case(
                            ((ActivityLog.activity_type == "click_search_submit", 1)),
                            else_=0,
                        )
                    ).label("estimateVolume"),
                    func.sum(
                        case(
                            ((ActivityLog.activity_type == "click_history_search", 1)),
                            else_=0,
                        )
                    ).label("searchVolume"),
                )
                .filter(
                    ...
                    ActivityLog.activity_type.in_(
                        ["click_search_submit", "click_history_search"]
                    ),
                )
                .group_by(
                    cast(

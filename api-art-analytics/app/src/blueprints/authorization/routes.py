...
def login():
    ...
    if request.method == "POST":
        ...
        if user and check_password_hash(user.password, password):
            ...
            last_payment = (
                PaymentTrack.query.filter_by(user_id=user.id)
            ...
            if last_payment:
                ...
                response["remaining_closest_volume"] = (
                    last_payment.remaining_closest_volume
                )
                response["remaining_search_volume"] = (
                    last_payment.remaining_search_volume
                )
...
def signin():
    ...
    if request.method == "POST":
        ...
        if len(password) > 20:
            return (
                jsonify({"error": "Password should have fewer than 20 characters"}),
                401,
            )
        ...
        if user:
            ...
            try:
                ...
                payment = PaymentTrack(
                    ...
                    plan_end_date=datetime.today()
                    + timedelta(days=config.product["free_plan_days"]),
                ...
                try:
                    confirm_url = url_for(
                        "authorization.confirm_email", token=token, _external=True
                    )
                    html = confirmation_email_html(new_user, confirm_url)
                    msg = Message(
                        "Artyx: Confirmation de votre email",
                        recipients=[email],
                        html=html,
                    )
                    mail.send(msg)
...
def reset_password():
    ...
    if user:
        ...
        reset_url = url_for(
            "authorization.set_new_password", token=token, _external=True
        )
        html = reset_email_html(user, reset_url)
        msg = Message(
            "Artyx: Réinitialisation du mot de passe", recipients=[email], html=html
        )
        mail.send(msg)
        return (

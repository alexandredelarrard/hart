    ...
    def homogenize_answers(self, df_done):
        ...
        for col in ["OBJECT_CATEGORY", "OBJECT_SUB_CATEGORY"]:
            if col in df_done.columns:
                ...
                df_done[col] = df_done[col].swifter.apply(
                    lambda x: self.nlp.stemSentence(str(x).split(" "))
                )

        df_done["CLASSE"] = np.where(
            df_done["OBJECT_CATEGORY"].isin(
                [
                    "jewelri",
                    "jewelleri",
                    "furnitur",
                    ...
                    "ornament",
                    "object of vertu",
                    "furnitur and decor art",
                    "meubl ancien et de style",
                    "art decoratif",
                    "object d'art",
                    "art de la tabl",
                    "jewel",
        ...
        value_counts.loc[value_counts != 1].to_csv(
            "D:/data/default/sub_cat_value_counts.csv", sep=";"
        )

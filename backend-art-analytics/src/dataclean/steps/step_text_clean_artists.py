...
from src.utils.utils_crawler import read_crawled_pickles, read_crawled_csvs
...
class StepTextCleanArtists(TextCleaner):
    ...
    def get_name_from_text(self, df):
        ...
        df[self._artist.artist_family_name] = df["ARTIST_NAME"].apply(
            lambda x: extract_upper(word_tokenize(x))
        )
        df[self._artist.artist_surname] = df[
            ["ARTIST_NAME", self._artist.artist_family_name]
        ].apply(lambda x: x[0].replace(x[1], "").strip(), axis=1)

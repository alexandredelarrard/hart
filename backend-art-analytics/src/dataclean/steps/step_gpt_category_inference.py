...
class StepCategoryGptInference(GPTCleaner):
    ...
    def __init__(self, 
        ...
        self.save_queue_path= "/".join([self._config.gpt.save_path, self.category])
        self.clean_info_with_mapping = self._config.evaluator.info_to_mapping
        self.mappings = self._config.evaluator.cleaning_mapping
        self.category_mapping_path = self._config.evaluator.category_mapping_path
    ...
    def extract_features(self, df_done):
        ...
        df_done["ANSWER"] = df_done["ANSWER"].swifter.apply(lambda x: 
                                            homogenize_keys_name(x, self.col_mapping, smooth_text=False))
    ...
    def remove_missing(self, df_done):
        ...
        df_done["FRENCH_TITLE"] = df_done["FRENCH_TITLE"].apply(lambda x: str(x).replace("Description de l'art:", ""))
        df_done["ENGLISH_TITLE"] = df_done["ENGLISH_TITLE"].apply(lambda x: str(x).replace("Art Description:", ""))

        df_done["FRENCH_TITLE"] = np.where(df_done["FRENCH_TITLE"].isin(["Sans titre", "Description de l'art", "Sans Titre", "Description de l'art:"]), np.nan, df_done["FRENCH_TITLE"]) 
        df_done["ENGLISH_TITLE"] = np.where(df_done["ENGLISH_TITLE"].isin(["Untitled", "Art Description", "Art Description:"]), np.nan, df_done["ENGLISH_TITLE"])

        # remove all mvs in any column
        for col in df_done.columns:
            df_done = df_done.loc[df_done[col].notnull()]

        self._log.info(f"Removing {shape_0 - df_done.shape[0]}/{shape_0} observations due to missing info")
        return df_done.reset_index(drop=True)
    
    def apply_mapping_func(self, vector, mapping_dict):
        return vector.swifter.apply(lambda x: self.exact_map_value_to_key(str(x).replace("\"-\"", "\" \"").replace("\",\"","\"\""), mapping_dict))
    
    @timing
    def clean_category(self, df_done):

        # clean text in each category
        truncated_cat = df_done["OBJECT_CATEGORY"].apply(lambda x: self.pseudo_clean_category(x))
        df_done["CLEAN_OBJECT_CATEGORY"] = self.apply_mapping_func(truncated_cat, self.cat_map)
        self._log.info((f"Remaining missing mapping proportion \
    ...
    def save_infos_to_tables(self, df_done):
        ...
        df_done["STATUS"] = np.where(df_done["CLEAN_OBJECT_CATEGORY"].isnull(), "KO", "OK")
        self.remove_rows_sql_data(values=df_done[self.name.id_item].tolist(),
                                    column=self.name.id_item,
                                    table_name="GPT_TRANSLATE_CATEGORIZE")
        self.write_sql_data(dataframe=df_done,
                            table_name="GPT_TRANSLATE_CATEGORIZE",
                            if_exists="append")

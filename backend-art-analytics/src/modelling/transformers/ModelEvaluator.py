...
class ModelEvaluator(object):
    ...
    def evaluate_model(self, model_parameters, y_prediction, y_true):
        ...
        if model_parameters["objective"] == "regression":
            ...
            if metrique == "mape":
                error = metrics.mean_absolute_percentage_error(y_true, y_prediction)*100
            elif metrique == "rmse":
            ...
            logging.info(f"EVALUATION : ERROR {metrique}= {error:.2f} | BIAS= {mean_bias:.2f} | ABS_BIAS= {mean_absolute_bias:.2f}")
            logging.info("_" * 60)
        else: 

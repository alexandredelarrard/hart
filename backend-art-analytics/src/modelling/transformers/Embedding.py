...
from src.constants.variables import PICTURE_TYPE, TEXT_TYPE_FR, TEXT_TYPE_EN
...
class StepEmbedding(Step):
    ...
    def __init__(self, context: Context, config: DictConfig, type: str = "picture"):
        ...
        if type in [TEXT_TYPE_FR, TEXT_TYPE_EN]:
            ...
            raise Exception(
                f"Can only handle text_en, text_fr or picture so far. No Audio & co as embeddings. value is {type}"
            )
    ...
    def get_picture_embedding(self, picture_path):
        ...
        if isinstance(picture_path, str):
            ...
            raise Exception(
                f"Text need to be str or List[str] to be embedded intead of {picture_path.dtype}"
            )

...
class StepFillDBEmbeddings(Step):
    ...
    def __init__(self, 
        ...
        self.step_embedding = StepEmbedding(context=context, 
                                            config=config, 
                                            type=self.type)
        self.data_retreiver = DatasetRetreiver(context=context, 
                                               config=config)
        self.postgres = FillDBEmbeddings(context=self._context,
                                        config=config, 
                                        type=self.type)
    ...
    def run(self):
        ...
        #save to chroma db
        self.postgres.save_collection(df_desc.to_dict(orient="records"), 
                                      results)
    ...
    def get_data(self):
        if self.type == PICTURE_TYPE:
            ...
            df_desc = self.data_retreiver.get_all_text(data_name=self.full_data, 
                                                       language=langue, 
                                                       limit=3100000)
            
            # create output var
            self.vector= self.name.total_description
            df_desc[self.vector] = (df_desc[langue + "_TITLE"] + 
                                    " " + 
                                    df_desc[langue + "_DESCRIPTION"])
    ...
    def filter_out_embeddings_done(self, df_desc):
        ...
        self._log.info(f"DONE IDS in Postgres DB = {len(done_ids)}, TODO = {df_desc.shape[0]}")
        return df_desc

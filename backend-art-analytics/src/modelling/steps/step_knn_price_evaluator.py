...
class StepKNNPriceEvaluator(Step):
    ...
    def predict(self):
        ...
        sub_df = df.loc[df[self.name.total_description].apply(lambda x: " vase " in " " + str(x).lower() + " ")].sample(frac=0.25)
        liste_text = sub_df[self.name.total_description].apply(lambda x : x.lower()).tolist()
        liste_ids_tested = sub_df[self.name.id_item].tolist()

        embeddings = self.step_embedding.get_text_embeddings(liste_text, 
        ...
        df_estimate = df_results[["ID_ITEM_ORIGIN", self.name.eur_min_estimate, 
                            self.name.eur_max_estimate, self.name.eur_item_result,
                            "DISTANCE"]].groupby("ID_ITEM_ORIGIN").aggregate(
                                {
        ...
        df_estimate = df_estimate.merge(sub_df[[self.name.id_item, self.name.eur_min_estimate, 
                                             self.name.eur_max_estimate, self.name.eur_item_result]],
                                            ...
                                            validate="m:1",
                                            suffixes=("KNN_", ""))
        df_estimate[self.name.auctionner_estimate] = df_estimate[[self.name.eur_max_estimate,

                                                        self.name.eur_min_estimate]].mean(axis=1)
        df_estimate = df_estimate.loc[df_estimate[self.name.eur_item_result] >= 10]

        df_estimate["TEST"] = df_estimate[[self.name.eur_min_estimate+"KNN_", 
                                           self.name.eur_max_estimate+"KNN_"]].mean(axis=1)
        
        # analyse results
        result_bias = self.evaluator.get_mean_bias(y_prediction=df_estimate[self.name.eur_item_result+"KNN_"],
                                                   y_true=df_estimate[self.name.eur_item_result])

        result_mape = self.evaluator.evaluation_mape_mean(y_prediction=df_estimate[self.name.eur_item_result+"KNN_"],
                                                          y_true=df_estimate[self.name.eur_item_result])
        test_mape = self.evaluator.evaluation_mape_mean(y_prediction=df_estimate["TEST"],
                                                          y_true=df_estimate[self.name.eur_item_result])
        min_mape = self.evaluator.evaluation_mape_mean(y_prediction=df_estimate[self.name.eur_min_estimate+"KNN_"]+1,
                                                        y_true=df_estimate[self.name.eur_min_estimate]+1)
        max_mape = self.evaluator.evaluation_mape_mean(y_prediction=df_estimate[self.name.eur_max_estimate+"KNN_"]+1,
                                                        y_true=df_estimate[self.name.eur_max_estimate]+1)
        
        result_std = self.evaluator.evaluation_mape_std(y_prediction=df_estimate[self.name.eur_item_result+"KNN_"],
                                                        y_true=df_estimate[self.name.eur_item_result]+1)
        
        auctionner_mape = self.evaluator.evaluation_mape_mean(y_prediction=df_estimate[self.name.auctionner_estimate],
                                                            y_true=df_estimate[self.name.eur_item_result])
    ...
    def clean_text_results(self, liste_ids_tested, text_results):
        ...
        df_results = df_results.loc[df_results[self.name.id_item] != df_results["ID_ITEM_ORIGIN"]]

        for col in [self.name.eur_min_estimate, self.name.eur_max_estimate, self.name.eur_item_result]:
            df_results[col] = df_results[col].replace("", 0).astype(float).replace(0,np.nan)

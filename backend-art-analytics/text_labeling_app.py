...
class WebAPP(Step):
    ...
    def instatiate_session(self, st):
        ...
        if "options" not in st.session_state:
            st.session_state["options"] = list(
                np.sort(list(df_done[1].unique()))
            )  # self.class_options +
    ...
    def get_display(self, st, col1, col2, col3):
        ...
        try:
            col1.image(
                f"D:/data/{row["SELLER"]}/pictures/{row["ID_PICTURE"]}.jpg", width=350
            )
        except Exception:
            pass
        ...
        self.user_input = col3.selectbox(
            f'Proba: {row["PROBA_0"]}/ Options',
        ...
        if self.user_input_new:
            st.session_state["options"] = list(
                np.sort(st.session_state["options"] + [self.user_input_new])
            )

        self.id_item = row["ID_ITEM"]
        col3.write(
            f"ROW ID: {st.session_state['index']}/{len(st.session_state['data'])}"
        )

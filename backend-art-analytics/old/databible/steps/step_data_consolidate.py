...
class StepDataConsolidate(Step):
    ...
    def consolidate_communes(self):
        ...
        ].drop_duplicates("ID_COMMUNE")

        commune_tables = [
            x
            for x in self._tables_in_sql
            if "CLEAN" not in x
            and "CONSOLIDATE" not in x
            and "COMMUNE" in x
            and "ENCODAGE" not in x
        ]
        ...
        for table_name in commune_tables:
            ...
            df_root = df_root.merge(
                df_table.drop(["NOM_COMMUNE_ORIGINE"], axis=1),
                on="ID_COMMUNE",
                how="left",
                validate="1:1",
            )
    ...
    def consolidate_zone_emploi(self):
        ...
        zone_emploi_tables = [
            ...
            if "CLEAN" not in x
            and "CONSOLIDATE" not in x
            and "ZONE_EMPLOI" in x
            and "CHOMAGE" not in x
            and "COMMUNE_" not in x
        ]
        ...
        for table_name in zone_emploi_tables:
            df_table = self.read_sql_data(table_name)
            df_ze = df_ze.merge(
                df_table.drop(["NOM_ZONE_EMPLOI"], axis=1),
                on="ID_ZONE_EMPLOI",
                how="left",
                validate="1:1",
            )

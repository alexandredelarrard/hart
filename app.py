...
class WebAPP(Step):
    ...
    def run(self):
        ...
        if widget_ui["button"]:
            ...
            if widget_ui["query_text"]:
                ...
                embeddings = st.session_state["text_embedding"].get_text_embedding(widget_ui["query_text"])
                text_results = st.session_state["text_collection"].query_collection(embeddings)

            if widget_ui["picture_paths"]:
                self.picture_path = f"D:/data/drouot/pictures/{widget_ui["picture_paths"].name}"
                embeddings = st.session_state['picture_embedding'].get_picture_embedding(self.picture_path)
                picture_results = st.session_state["picture_collection"].query_collection(embeddings)
    ...
    def instatiate_session(self):
        ...
        if 'text_embedding' not in st.session_state:
            st.session_state['text_embedding'] = StepEmbedding(context=context, config=config, type="text")

        if "picture_embedding" not in st.session_state:
            st.session_state['picture_embedding'] = StepEmbedding(context=context, config=config, type="picture")
    ...
    def display_text_results(self, tab1, text_results):
        ...
        with tab_col2.container(height=400, border=False):
            st.header(f"Estimations  : {estim_min.median():.0f} - {estim_max.median():.0f} EUR ")
    ...
    def info_in_container(self, doc, document, distance):
        if "MISSING" not in doc[self.name.id_picture]:
            try:
                st.image(f"D:/data/{doc[self.name.seller]}/pictures/{doc[self.name.id_picture]}.jpg", 
                        caption=f"Date: {doc[self.name.date]} / Distance : {distance:0.2f} ")
                st.write(f"Resultat: {doc[self.name.item_result]} / " + f"Estimations: {doc[self.name.min_estimate]}-{doc[self.name.max_estimate]} {doc[self.name.currency]}")
                st.write(document)

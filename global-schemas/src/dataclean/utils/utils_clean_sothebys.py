...
def get_max_def(x):
    ...
    df = dict()
    for value in liste:
        df[int(value.split(" ")[1].replace(",", "").replace("w", ""))] = value.split(
            " "
        )[0].strip()
...
class CleanSothebys(TextCleaner):
    ...
    def get_id_auction(self, df):
        return df[self.name.url_auction].apply(
            lambda x: str(x)
            .split("auction/")[-1]
            .split("auctions/")[-1]
            .split(".html")[0]
    ...
    def clean_auctions(self, df_auctions):
        ...
        df_auctions[self.name.type_sale] = (
            df_auctions[self.name.type_sale].str.replace("CATEGORY:\n", "").str.lower()
        )
        return df_auctions
    ...
    def extract_hour_infos(self, df):
        ...
        # date, place, maison
        df[self.name.localisation] = self.get_list_element_from_text(
            df[self.name.date], liste=localisation
        )
        ...
        df[self.name.date] = pd.to_datetime(
            df[self.name.date], format="%d %B %Y", exact=False
        )
    ...
    def clean_items_per_auction(self, df):
        ...
        df[self.name.lot] = df[self.name.item_description].apply(
        ...
        df[self.name.auction_title] = df[self.name.item_file].apply(

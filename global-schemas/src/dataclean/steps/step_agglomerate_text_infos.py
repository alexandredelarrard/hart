...
from src.constants.variables import liste_currency_paires, fixed_eur_rate, DATE_FORMAT
...
class StepAgglomerateTextInfos(TextCleaner):
    ...
    def __init__(self, context: Context, config: DictConfig, mode: str = "history"):
    ...
    def run(self):
        ...
        df = self.remove_missing_values(
            df, important_cols=[self.name.total_description]
        )
    ...
    def write_data(self, df):
        if self.mode == "history":
            ...
            self._log.info(
                f"Appending {df.shape} to the already existing history table {self.sql_table_name}"
            )
            ...
            self.write_sql_data(
                dataframe=df, table_name=self.sql_table_name, if_exists="append"
            )
    ...
    def write_data_per_item(self, df):
        if self.mode == "history":
            ...
            self._log.info(
                f"Appending {df.shape} to the already existing history table {self.sql_table_name_per_item}"
            )
    ...
    def homogenize_localisation(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df[self.name.localisation] = np.where(
            ...
            mapped_loc,
            np.where(
                df[self.name.localisation] == "nan", np.nan, df[self.name.localisation]
            ),
        )
    ...
    def reduce_dim_to_per_item(self, df):
        serie_list_pict = (
            df[[self.name.id_item, self.name.id_picture]]
            .groupby(self.name.id_item)[self.name.id_picture]
            .apply(list)
        )
        new_df = df.drop_duplicates(self.name.id_item)
        new_df = new_df.drop([self.name.id_unique, self.name.id_picture], axis=1).merge(
            serie_list_pict,

...
class TextCleaner(Step):
    ...
    def clean_id_picture(self, df: pd.DataFrame, limite: int = 100, paths: Dict = {}):
        ...
        for col in [self.name.url_picture, self.name.id_picture]:
            if col not in df.columns:
                raise Exception(
                    f"{col} not in df, cannot continue to clean {self.name.id_picture}"
                )
        ...
        liste_pictures_missing = (
            df[self.name.id_picture]
            .value_counts()
            .loc[df[self.name.id_picture].value_counts() > limite]
            .index
        )
        self._log.info(
            f"SET PICTURES ID TO MISSING FOR {len(liste_pictures_missing)} picts having more than {limite} picts"
        )

        df[self.name.id_picture] = np.where(
            df[self.name.id_picture].isnull(),
            "NO_PICTURE",
            np.where(
                df[self.name.id_picture].isin(list(liste_pictures_missing)),
                "FAKE_PICTURE",
                df[self.name.id_picture],
            ),
        )
    ...
    def extract_currency(self, df):
        ...
        if sum(df[self.name.min_estimate].isnull()) > sum(
            df[self.name.item_result].isnull()
        ):
        ...
        if (
            currency_col == self.name.brut_estimate
            and self.name.brut_result in df.columns
        ):
            currency_brut_result = self.get_list_element_from_text(
                df[self.name.brut_result]
            )
        ...
        if (
            currency_col == self.name.brut_result
            and self.name.brut_estimate in df.columns
        ):
            currency_brut_estimate = self.get_list_element_from_text(
                df[self.name.brut_estimate]
            )
            df[self.name.currency] = np.where(
                df[self.name.currency].isnull(),
    ...
    def add_complementary_variables(self, df, seller):
        df[self.name.id_item] = df[self.name.url_full_detail].apply(
            lambda x: encode_file_name(str(x))
        )
    ...
    def extract_estimates(self, df):
        ...
        if (
            self.name.brut_estimate not in df.columns
            and self.name.brut_result not in df.columns
        ):
            raise Exception(
                f"Need to provide either {self.name.brut_estimate} or {self.name.brut_result} in the dataframe toe deduce price estimate"
            )

        if self.name.brut_result not in df.columns:
            self._log.warning(
                f"{self.name.brut_result} not in df columns, will take {self.name.brut_estimate} as proxy for price estimate"
            )
            df[self.name.brut_result] = df[self.name.brut_estimate]

        df[self.name.item_result] = self.get_estimate(
            df[self.name.brut_result], min_max="min"
        )

        if self.name.brut_estimate not in df.columns:
            self._log.warning(
                f"{self.name.brut_estimate} not in df columns, will take {self.name.brut_result} as proxy for price estimate"
            )
            df[self.name.brut_estimate] = df[self.name.brut_result]

        df[self.name.min_estimate] = self.get_estimate(
            df[self.name.brut_estimate], min_max="min"
        )
        df[self.name.max_estimate] = self.get_estimate(
            df[self.name.brut_estimate], min_max="max"
        )
    ...
    def clean_detail_infos(self, df_detailed):
        ...
        df_detailed[self.name.detailed_description] = np.where(
            lowered.isin(LISTE_WORDS_REMOVE),
            np.nan,
        ...
        if self.name.url_picture in df_detailed.columns:
            try:
                df_detailed = df_detailed.sort_values(
                    self.name.url_picture, ascending=False
                )
    ...
    def create_unique_id(self, df):
        df = df.sort_values(
            [self.name.url_full_detail, self.name.id_picture], ascending=[0, 0]
        )
        df[self.name.id_unique] = (
            df[self.name.id_item] + "_" + df[self.name.id_picture]
        ).apply(lambda x: encode_file_name(x))

        shape_0 = df.shape[0]
        df = df.drop_duplicates(self.name.id_unique)
        self._log.info(
            f"DROPPING {shape_0-df.shape[0]} rows due to duplicate {self.name.id_unique}"
        )
    ...
    def homogenize_lot_number(self, df):
        ...
        df["ONE"] = 1
        min_lot = df[
            [self.name.url_auction, self.name.id_item, "ONE"]
        ].drop_duplicates()
        min_lot[self.name.lot] = (
            min_lot[[self.name.url_auction, "ONE"]]
            .groupby(self.name.url_auction)
            .cumsum()
        )
        df = df.merge(
            min_lot,
            on=[self.name.url_auction, self.name.id_item],
            how="left",
            validate="m:1",
            suffixes=("_ORIGIN", ""),
        )

...
from src.constants.variables import CHROMA_PICTURE_DB_NAME, CHROMA_TEXT_DB_NAME
...
class ChromaCollection(Step):
    ...
    def __init__(
        ...
        if type == "picture":
            self.collection = chroma_db.get_or_create_collection(
                name=CHROMA_PICTURE_DB_NAME, metadata={"hnsw:space": "cosine"}
            )
        elif type == "text":
            self.collection = chroma_db.get_or_create_collection(
                name=CHROMA_TEXT_DB_NAME, metadata={"hnsw:space": "cosine"}
            )
        else:
    ...
    @timing
    def save_collection(
        self, df_desc: pd.DataFrame, embeddings: np.array, is_document: bool = True
    ) -> None:
        ...
        for i in range(nbr_steps):
            self._log.info(
                f"[DB EMBEDDING] : adding batch {i+1} / {nbr_steps} to chromadb"
            )
    ...
    def query_collection(self, query_embedded: np.array) -> Dict:
        return self.collection.query(
            query_embeddings=query_embedded, n_results=self.n_top_results
        )
    ...
    def delete_collection(self, list_ids):
        ...
        for i in range(nbr_steps):
            self._log.info(
                f"[DB EMBEDDING] : Deleting batch {i+1} / {nbr_steps} to chromadb"
            )

...
from src.constants.variables import TEXT_DB_FR, TEXT_DB_EN
...
class StepTextClustering(Step):
    ...
    def __init__(
        ...
        self.step_embedding = StepEmbedding(
            context=context, config=config, type=[TEXT_DB_EN]
        )
    ...
    def run(self, data_name: str, vector: str):
        ...
        self.embeddings = self.step_embedding.get_text_embedding(
            df_desc[self.vector].tolist(), prompt_name=self.prompt_name
        )

        if self.to_cluster:
            self.reduced_embedding = self.step_cluster.embedding_reduction(
                self.embeddings, method_dim_reduction="umap"
            )

            # get cluster
            df_desc[self.name.cluster_id] = self.step_cluster.hdbscan_clustering(
                self.reduced_embedding
            )
    ...
    def plot_2d_embeddings(self, df_desc):
        plot_reduced_embedding = self.step_cluster.embedding_reduction(
            self.embeddings, method_dim_reduction="tsne"
        )
    ...
    def save_pictures_for_test(self):
        ...
        sub_df["TO"] = sub_df[self.name.cluster_id].apply(
            lambda x: "D:/data/other/" + "vase_" + str(x)
        )
    ...
    def get_top_words_cluster(self, df_desc):
        liste_docs = (
            df_desc[[self.name.cluster_id, self.vector]]
            .groupby(self.name.cluster_id)[self.vector]
            .apply(lambda x: " ".join(x))
        )
        self.mapping_words_cluster = self.nlp.extract_words_per_cluster(
            liste_docs, ngram_range=(1, 2), n_top_words=self.n_words_cluster
        )

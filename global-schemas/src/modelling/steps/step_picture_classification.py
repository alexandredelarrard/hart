...
from src.utils.utils_crawler import read_json, save_json
...
class StepPictureClassification(Step):
    ...
    def __init__(self, context: Context, config: DictConfig, save_model: bool = True):
        ...
        self.fine_tuned_model_name = self._context.paths["MODEL"] / Path(
            self._config.picture_classification.fine_tuned_model_path
        )
        self.default_image_path = self._context.paths["DEFAULT"] / Path(
            self._config.picture_classification.default_image_path
        )
        self.save_pict_test_path = self._context.paths["ROOT"] / Path("test")
    ...
    def training(self):
        ...
        if self.save_model:
            picture_model.save_model(self.fine_tuned_model_name)
            save_json(
                self.classes_2id,
    ...
    def predicting(self, view_name="PICTURES_CATEGORY_07_06_2024_286"):
        ...
        df_desc[self.name.id_picture] = [
            x[self.name.id_picture] for x in collection_infos["metadatas"]
        ]
        ...
        pict_transformer = picture_model.load_trained_model(
            model_path=self.fine_tuned_model_name
        )
        test_dataset = ArtDataset(
            df_desc["pict_path"].tolist(),
            self.classes_2id,
            transform=pict_transformer,
            mode="test",
            default_path=self.default_image_path,
        ...
        self.write_sql_data(dataframe=answers, table_name=view_name, if_exists="append")
    ...
    def clean_list_pictures(self, df, df_done):
        ...
        df = df.loc[
            ~df[self.name.id_unique].isin(df_done[self.name.id_unique].tolist())
        ]
        return df
    ...
    def save_pictures_to_folders(self, answers):
        ...
        sub_answers["save_path"] = sub_answers["TOP_0"].apply(
            lambda x: self.save_pict_test_path / Path(x)
        )
    ...
    def get_train_test_data(self, ratio_validation):
        ...
        pictures_paths = glob.glob(self.picture_path / Path("*/*.jpg"))

        random.shuffle(pictures_paths)
        train_volume = int(len(pictures_paths) * (1 - ratio_validation))
        self._log.info(
            f"TRAIN DATA VOLUME = {train_volume} / VAL VOLUME = {len(pictures_paths) - train_volume}"
        )

        data = {
            "train": pictures_paths[:train_volume],

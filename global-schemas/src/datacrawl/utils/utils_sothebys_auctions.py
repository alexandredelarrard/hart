...
class SothebysAuctions(Crawling):
    ...
    def __init__(self, context: Context, config: DictConfig):
        ...
        self.ref = {
            "date": pd.to_datetime("2006-01-01", format=DATE_FORMAT),
            "value": 1136156400000,
            "step_size": 86400000,
        }
    ...
    def new_urls(self, start_date, end_date, url_auctions):
        ...
        for year in range(start_year, end_date.year + 1):
            ...
            for month in range(start_month, end_month):
                start_date_range = pd.to_datetime(
                    f"{year}-{month}-01", format=DATE_FORMAT
                )
                ...
                start_date_range = end_date_range.strftime("%m-%d-%Y").replace(
                    "-", "%2F"
                )
                end_date_range = end_date_range.strftime("%m-%d-%Y").replace("-", "%2F")
                to_crawl.append(
                    url_auctions
                    + f"from={end_date_range}&to={start_date_range}&f0={lower_ref}-{higher_ref}&q="
                )
    ...
    def load_all_page(self, driver, config: Dict):
        ...
        liste_lots = []
        counter_pages = 0
        new_liste_lots = self.get_elements(

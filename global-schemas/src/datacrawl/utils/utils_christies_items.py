...
class ChristiesItems(Crawling):
    ...
    def __init__(self, context: Context, config: DictConfig):
        ...
        self.correction_urls_auction = self.root_path / Path(
            self._config.crawling["christies"].correction_urls_auction
        )
    ...
    def urls_to_crawl(self, df_auctions):
        ...
        df_auctions[self.name.url_auction] = np.where(
            df_auctions[self.name.url_auction].apply(lambda x: "sso?" in x),

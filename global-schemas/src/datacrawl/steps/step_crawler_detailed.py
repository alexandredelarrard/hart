...
class StepCrawlingDetailed(Crawling):
    ...
    def __init__(
        ...
        self.sql_table_name = TextCleaner(
            context=context, config=config
        ).get_sql_db_name(self.seller, mode)
    ...
    def get_list_items_to_crawl(self):
        ...
        df = df.sort_values(
            [self.name.url_full_detail, self.name.brut_result], ascending=[0, 0]
        )
        to_crawl = (
            df.loc[df[self.name.url_full_detail].notnull(), self.name.url_full_detail]
            .drop_duplicates()
            .tolist()
        )
    ...
    def specificity_seller(self, df):
        ...
        if len(df[self.name.item_title].shape) == 2:
            title = (
                df[self.name.item_title].fillna("").apply(lambda x: " ".join(x), axis=1)
            )
            df = df.drop(self.name.item_title, axis=1)
            df[self.name.item_title] = title

        df[self.name.lot] = df[self.name.item_title].apply(
            lambda x: str(x).split(".")[0].replace("No reserve\n", "")
        )

        # error of url full detail need to be corrected
        df = df.loc[df[self.name.url_full_detail].notnull()]
        df[self.name.url_full_detail] = df[
            [self.name.url_full_detail, self.name.lot]
        ].apply(
            lambda x: re.sub(
                "lot.(\\d+)+",
                f"lot.{x[self.name.lot]}",
                str(x[self.name.url_full_detail]),
            ),
            axis=1,
        )
    ...
    def extract_valid_crawled(self, df_crawled):
        ...
        if df_crawled.shape[0] != 0:
            df_crawled = df_crawled.loc[
                df_crawled[self.name.detailed_description].notnull()
            ]
            if self.recrawl_pictures:
                df_crawled = self.recrawl_pictures_missing(df_crawled)
            already_crawled = (
                df_crawled[self.name.url_full_detail].drop_duplicates().tolist()
            )
        else:
            already_crawled = []
    ...
    def recrawl_pictures_missing(self, df_crawled):
        ...
        if self.name.url_picture in df_crawled.columns:
            df_crawled[self.name.url_picture] = np.where(
                df_crawled[self.name.url_picture].apply(
                    lambda x: "data:image/svg+xml;base64," in x or x == []
                ),
                np.nan,
            ...
            df_crawled = df_crawled.loc[
                df_crawled[self.name.url_picture].notnull()
                & (df_crawled[self.name.id_picture] != "FAKE_PICTURE")
            ]

...
class StepCrawlingAuctions(Crawling):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            threads=threads,
            save_queue_path=self.paths["auctions"],
            kwargs=kwargs,
        )
    ...
    def get_auctions_urls_to_crawl(self) -> List[str]:
        ...
        to_crawl = self.seller_utils.urls_to_crawl(
            self.start_date, self.end_date, self.url_auctions
        )
        already_crawled = get_files_done_from_path(
            file_path=self.paths["auctions"], url_path=self.url_auctions
        )
    ...
    def crawling_auctions_iteratively(self, driver):
        ...
        save_infos(df_infos, path=self.paths["auctions"] / Path(f"{query}.csv"))
